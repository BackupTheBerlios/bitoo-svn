/*
 * Copyright (c) 2004 Fabio Di Fabio All rights reserved.
 * 
...
public class TrackerProxy extends Thread {
	private final static Logger logger = Logger.getLogger(TrackerProxy.class
			.getName());
	...
	public TrackerProxy(String remoteTrackerBaseURL, String peerId,
			InetAddress localPeerAddress, int localPeerPort) throws IOException {
		super("TrackerProxy");
		...
		System.arraycopy(this.localPeerAddress.getAddress(), 0, localPeerBytes,
				0, 4);
	...
	public void run() {
		...
		try {
			...
			while (! isStopping()) {
				...
				HttpURLConnection conn = (HttpURLConnection) trackerRequestURL
						.openConnection();
				conn.connect();

				BufferedInputStream inReply = new BufferedInputStream(conn
						.getInputStream());
				...
				logger.debug("Real tracker response bytes: ["
						+ new String(allBuf) + "]");
			...
			if(isStopping()) {
				logger.warn("Error while stopping tracker proxy.", e);
			} else {
				logger.error("Error tracker proxy failed. Closing it", e);
			}
		}
	}
